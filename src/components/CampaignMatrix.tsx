...
export const CampaignMatrix: React.FC<CampaignMatrixProps> = memo(
  ({ campaignId: _campaignId, onRender }) => {
    ...
    // Memoize fields to prevent recreating on every render
    const fields = useMemo(() => defaultFields, []);

    // Memoize loadAssets to prevent recreation on every render
    ...
    const deleteRow = useCallback(
      (rowId: string) => {
        setRows(prev => {
          ...
          return prev.filter((r: any) => r.id !== rowId);
        });
      },
      [showNotification]
    );
    ...
    const toggleRowLock = useCallback((rowId: string) => {
      setRows(prev =>
        prev.map((row: any) => (row.id === rowId ? { ...row, locked: !row.locked } : row))
      );
    }, []);

    const toggleCellLock = useCallback((rowId: string, fieldId: string) => {
      setRows(prev =>
        prev.map((row: any) => {
          if (row.id === rowId) {
            return {
    ...
    const updateCell = useCallback((rowId: string, fieldId: string, updates: any) => {
      setRows(prev =>
        prev.map((row: any) => {
          if (row.id === rowId) {
            return {
    ...
    const openAssetDialog = useCallback((rowId: string, fieldId: string, type: string) => {
    ...
    const selectAsset = useCallback(
      ...
      [selectedAssetDialog, updateCell, showNotification]
    );

    // Memoize expensive combination generation logic
    ...
    }, [rows, maxCombinations, fields, showNotification]);
    ...
    }, [combinations, onRender, showNotification]);

    // Memoize field icon to prevent recreation
    ...
    // Memoize cell content to prevent unnecessary re-renders
    const getCellContent = useCallback(
      (row: MatrixRow, field: MatrixField) => {
      ...
      [assets, getFieldIcon, updateCell, toggleCellLock, openAssetDialog]
    );

    // Memoize filtered assets for dialog to prevent recreation on every render
    const filteredAssets = useMemo(() => {
      if (!selectedAssetDialog) return [];

      return assets.filter((asset: any) => {
        switch (selectedAssetDialog.type) {
          case 'image':
    ...
    }, [assets, selectedAssetDialog?.type]);
...
CampaignMatrix.displayName = 'CampaignMatrix';
