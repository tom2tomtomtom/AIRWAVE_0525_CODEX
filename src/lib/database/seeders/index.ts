...
export class DatabaseSeeder {
  ...
  private async seedProfiles(result: SeedResult, options: SeederOptions): Promise<void> {
    ...
    for (const profile of profiles) {
      try {
        ...
        result.errors.push(
          `Error creating profile ${profile.email}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
  }
  ...
  private async seedClients(result: SeedResult, options: SeederOptions): Promise<void> {
    ...
    for (const client of clients) {
      try {
        ...
        result.errors.push(
          `Error creating client ${client.name}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
  }
  ...
  private async seedProjects(result: SeedResult, options: SeederOptions): Promise<void> {
    ...
    for (const project of projects) {
      try {
        ...
        result.errors.push(
          `Error creating project ${project.name}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
  }
  ...
  private async seedCampaigns(result: SeedResult, options: SeederOptions): Promise<void> {
    ...
    for (const campaign of campaigns) {
      try {
        const { error } = await this.supabase.from('campaigns').insert(campaign);
        ...
        result.errors.push(
          `Error creating campaign ${campaign.name}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
  }
  ...
  private async seedFeatureFlags(result: SeedResult, options: SeederOptions): Promise<void> {
    ...
    for (const flag of flags) {
      try {
        const { error } = await this.supabase.from('feature_flags').insert(flag);
        ...
        result.errors.push(
          `Error creating feature flag ${flag.key}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
  }
  ...
  private getFeatureFlagSeeds(
    environment: string
  ): Omit<FeatureFlag, 'created_at' | 'updated_at'>[] {
    const baseFlags = [
      {
